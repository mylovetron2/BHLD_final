...
/**
 * Table class for bhld_view_nhanvien
 */
class bhld_view_nhanvien extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'bhld_view_nhanvien';
		$this->TableName = 'bhld_view_nhanvien';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`bhld_view_nhanvien`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// mapb
		$this->mapb = new DbField('bhld_view_nhanvien', 'bhld_view_nhanvien', 'x_mapb', 'mapb', '`mapb`', '`mapb`', 200, 50, -1, FALSE, '`mapb`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// manv
		$this->manv = new DbField('bhld_view_nhanvien', 'bhld_view_nhanvien', 'x_manv', 'manv', '`manv`', '`manv`', 3, 11, -1, FALSE, '`manv`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tennhanvien
		$this->tennhanvien = new DbField('bhld_view_nhanvien', 'bhld_view_nhanvien', 'x_tennhanvien', 'tennhanvien', '`tennhanvien`', '`tennhanvien`', 200, 50, -1, FALSE, '`tennhanvien`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "bhld_view_chungtu_danhan_final") {
			$detailUrl = $GLOBALS["bhld_view_chungtu_danhan_final"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_manv=" . urlencode($this->manv->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "bhld_view_nhanvienlist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`bhld_view_nhanvien`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "bhld_view_nhanvienlist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "bhld_view_nhanvienview.php")
			return $Language->phrase("View");
		elseif ($pageName == "bhld_view_nhanvienedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "bhld_view_nhanvienadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("bhld_view_nhanvienview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("bhld_view_nhanvienview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "bhld_view_nhanvienadd.php?" . $this->getUrlParm($parm);
		else
			$url = "bhld_view_nhanvienadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("bhld_view_nhanvienedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("bhld_view_nhanvienedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("bhld_view_nhanvienadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("bhld_view_nhanvienadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("bhld_view_nhanviendelete.php", $this->getUrlParm());
	}
