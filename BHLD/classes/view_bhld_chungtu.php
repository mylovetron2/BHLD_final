...
/**
 * Table class for view_bhld_chungtu
 */
class view_bhld_chungtu extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'view_bhld_chungtu';
		$this->TableName = 'view_bhld_chungtu';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`view_bhld_chungtu`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// mact
		$this->mact = new DbField('view_bhld_chungtu', 'view_bhld_chungtu', 'x_mact', 'mact', '`mact`', '`mact`', 200, 50, -1, FALSE, '`mact`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// manv
		$this->manv = new DbField('view_bhld_chungtu', 'view_bhld_chungtu', 'x_manv', 'manv', '`manv`', '`manv`', 3, 11, -1, FALSE, '`manv`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ngct
		$this->ngct = new DbField('view_bhld_chungtu', 'view_bhld_chungtu', 'x_ngct', 'ngct', '`ngct`', CastDateFieldForLike("`ngct`", 0, "DB"), 133, 10, 0, FALSE, '`ngct`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// mapb
		$this->mapb = new DbField('view_bhld_chungtu', 'view_bhld_chungtu', 'x_mapb', 'mapb', '`mapb`', '`mapb`', 200, 50, -1, FALSE, '`mapb`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ghichu
		$this->ghichu = new DbField('view_bhld_chungtu', 'view_bhld_chungtu', 'x_ghichu', 'ghichu', '`ghichu`', '`ghichu`', 200, 50, -1, FALSE, '`ghichu`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ghichu->Sortable = TRUE; // Allow sort
		$this->fields['ghichu'] = &$this->ghichu;

		// madm
		$this->madm = new DbField('view_bhld_chungtu', 'view_bhld_chungtu', 'x_madm', 'madm', '`madm`', '`madm`', 200, 50, -1, FALSE, '`madm`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`view_bhld_chungtu`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "view_bhld_chungtulist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "view_bhld_chungtuview.php")
			return $Language->phrase("View");
		elseif ($pageName == "view_bhld_chungtuedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "view_bhld_chungtuadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("view_bhld_chungtuview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("view_bhld_chungtuview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "view_bhld_chungtuadd.php?" . $this->getUrlParm($parm);
		else
			$url = "view_bhld_chungtuadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("view_bhld_chungtuedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("view_bhld_chungtuadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("view_bhld_chungtudelete.php", $this->getUrlParm());
	}
