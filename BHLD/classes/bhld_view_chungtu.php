...
/**
 * Table class for bhld_view_chungtu
 */
class bhld_view_chungtu extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'bhld_view_chungtu';
		$this->TableName = 'bhld_view_chungtu';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`bhld_view_chungtu`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// mact
		$this->mact = new DbField('bhld_view_chungtu', 'bhld_view_chungtu', 'x_mact', 'mact', '`mact`', '`mact`', 200, 50, -1, FALSE, '`mact`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// manv
		$this->manv = new DbField('bhld_view_chungtu', 'bhld_view_chungtu', 'x_manv', 'manv', '`manv`', '`manv`', 3, 11, -1, FALSE, '`manv`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ngct
		$this->ngct = new DbField('bhld_view_chungtu', 'bhld_view_chungtu', 'x_ngct', 'ngct', '`ngct`', CastDateFieldForLike("`ngct`", 0, "DB"), 133, 10, 0, FALSE, '`ngct`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ngct->Nullable = FALSE; // NOT NULL field
		$this->ngct->Required = TRUE; // Required field
		$this->ngct->Sortable = TRUE; // Allow sort
		$this->ngct->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['ngct'] = &$this->ngct;

		// mapb
		$this->mapb = new DbField('bhld_view_chungtu', 'bhld_view_chungtu', 'x_mapb', 'mapb', '`mapb`', '`mapb`', 200, 50, -1, FALSE, '`mapb`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ghichu
		$this->ghichu = new DbField('bhld_view_chungtu', 'bhld_view_chungtu', 'x_ghichu', 'ghichu', '`ghichu`', '`ghichu`', 200, 50, -1, FALSE, '`ghichu`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ghichu->Sortable = TRUE; // Allow sort
		$this->fields['ghichu'] = &$this->ghichu;

		// madm
		$this->madm = new DbField('bhld_view_chungtu', 'bhld_view_chungtu', 'x_madm', 'madm', '`madm`', '`madm`', 200, 50, -1, FALSE, '`madm`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "bhld_view_chungtu_danhan") {
			$detailUrl = $GLOBALS["bhld_view_chungtu_danhan"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_mact=" . urlencode($this->mact->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "bhld_view_chungtulist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`bhld_view_chungtu`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "bhld_view_chungtulist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "bhld_view_chungtuview.php")
			return $Language->phrase("View");
		elseif ($pageName == "bhld_view_chungtuedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "bhld_view_chungtuadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("bhld_view_chungtuview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("bhld_view_chungtuview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "bhld_view_chungtuadd.php?" . $this->getUrlParm($parm);
		else
			$url = "bhld_view_chungtuadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("bhld_view_chungtuedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("bhld_view_chungtuedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("bhld_view_chungtuadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("bhld_view_chungtuadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("bhld_view_chungtudelete.php", $this->getUrlParm());
	}
	...
	public function renderEditRow()
	{
		...
		$this->ngct->EditValue = FormatDateTime($this->ngct->CurrentValue, 8);
